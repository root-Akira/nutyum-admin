...
export default function SiteSettings() {
  ...
  const [form, setForm] = useState({
    ...
    currency: 'INR', gst_number: '', cod_enabled: true, cod_charge: 0,
  ...
  const saveMutation = useMutation({
    mutationFn: async () => {
      const payload = { ...form, social_links: form.social_links }
    ...
    <div className="max-w-2xl space-y-6">
      <div className="rounded-xl border border-[rgba(23,61,34,0.08)] bg-[#FFFEFB] p-6 space-y-4">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          <Input label="Store Name" value={form.store_name} onChange={e => update('store_name', e.target.value)} />
          <Input label="Email" type="email" value={form.store_email} onChange={e => update('store_email', e.target.value)} />
      ...
      <div className="rounded-xl border border-[rgba(23,61,34,0.08)] bg-[#FFFEFB] p-6 space-y-4">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          ...
          <Input label="COD Charge (₹)" type="number" value={form.cod_charge} onChange={e => update('cod_charge', Number(e.target.value))} />
        </div>
